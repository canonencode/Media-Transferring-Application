...
public class SqliteScanSinkStressTests
{
    ...
    [Fact]
    public void APathContainingALoneSurrogate_IsStillRecorded_AndTheLossIsAnnounced()
    {
        ...
        // give. This test originally asked for an exact round trip, which a
        // TEXT column cannot deliver - storing the raw UTF-16 bytes as a BLOB
        // would, at the cost of making every path query in the application
        // work on bytes instead of text, to accommodate a corrupt name.
        //
        // Of the reachable answers, refusing the row is the worst: it loses a
        // file the device really has, which is the one thing this project will
        // not do. So the row is written with the substitution the encoder makes
        // anyway, and the fact that this path can no longer be matched back to
        // the device is said out loud rather than left for someone to discover
        // when a copy mysteriously fails.
        using var db = new TempDatabase();
        ...
        // But the stored path is not the device's, and that was announced.
        Assert.NotEqual(path, db.Column("SELECT path FROM file").Single());
        Assert.Contains("[UNSTORABLE NAME]", captured.ToString());
        Assert.Contains("cannot be located", captured.ToString());
    }
    ...
    [Fact]
    public void TwoSinksScanningAtOnce_AreNotSupported_AndTheSecondSaysSoQuickly()
    {
        // Inverted from how this test was first written, because the behaviour
        // it asked for is not reachable from this design and would be wrong to
        // reach for.
        //
        // SQLite allows one writer at a time, and this sink deliberately holds
        // a write transaction across a batch - that batching is what makes a
        // killed scan leave its rows behind instead of nothing, verified with a
        // real process kill. Interleaving two sinks on one thread therefore
        // deadlocks by construction: the second waits for a lock the first will
        // not release until it is allowed to continue. Dropping the batching to
        // allow concurrent writers would trade a proven durability property for
        // a scenario that does not exist yet - the scanner records one device
        // per process.
        //
        // So the supported behaviour is: one scan at a time, and the second one
        // fails quickly rather than appearing to freeze. What must NOT happen
        // is the second scan claiming success while recording nothing.
        using var db = new TempDatabase();
        ...
        Assert.ThrowsAny<Exception>(() =>
        {
            ...
            b.OnFile(FileObject("b.jpg"), "/B/b.jpg", FileKind.MediaFile, recovered: false);
            b.OnScanFinished(CleanOutcome() with { MediaFiles = 1, TotalFilesSeen = 1 });
        });
        stopwatch.Stop();

        Assert.True(stopwatch.Elapsed < TimeSpan.FromSeconds(5),
            $"the second sink took {stopwatch.Elapsed.TotalSeconds:F1}s to report the conflict");

        // The first scan is untouched by the second one's failure.
        a.OnScanFinished(CleanOutcome() with { MediaFiles = 20, TotalFilesSeen = 20 });
        Assert.Equal(20L, db.Count("file"));
        Assert.Equal(1L, db.Count("scan", "status = 'complete'"));
    }
    ...
    [Fact]
    public void ASerialDifferingOnlyByCase_IsDeliberatelyTreatedAsADifferentDevice()
    {
        // Inverted from how this test was first written, on purpose.
        //
        // Folding case would merge two device histories, and merging is the
        // worse direction: a split history is recoverable - the user sees two
        // entries for one phone - while a history that has absorbed another
        // phone's files reports those files as deleted from this one. The
        // padding case above is different, because padding is a transport
        // artifact rather than part of the serial.
        //
        // It is also not safely doable by case alone. ToUpperInvariant maps the
        // Turkish dotless 'ı' onto 'I', so "SERı123" and "SERI123" - two
        // genuinely different serials - would collapse into one. The test
        // immediately below this one exists to pin exactly that.
        using var db = new TempDatabase();
        ...
        Assert.Equal(2L, db.Count("device"));
    }
